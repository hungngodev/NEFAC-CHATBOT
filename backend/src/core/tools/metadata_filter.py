...
def filter_documents_by_metadata(documents: List[Document], filters: Dict[str, Any]) -> List[Document]:
    ...
    for doc in documents:
        # Determine the metadata schema based on 'file_type_category' or other indicators
        # For now, we'll assume 'file_type_category' or 'source' can help
        # In a real scenario, you might have a more robust way to map metadata to schema
        doc_metadata = doc.metadata

        # Check if all filters match
        all_filters_match = True
        for filter_key, filter_value in filters.items():
            # Handle nested metadata for author and categories
            if filter_key == "author_name":
                if not (doc_metadata.get("author") and doc_metadata["author"].get("name") == filter_value):
                    all_filters_match = False
                    break
            elif filter_key == "category_name":
                if not (doc_metadata.get("categories") and any(cat.get("name") == filter_value for cat in doc_metadata["categories"])):
                    all_filters_match = False
                    break
            elif filter_key == "tags":
                # Assuming filter_value for tags is a list, and we check if any tag matches
                if not (doc_metadata.get("tags") and any(tag in doc_metadata["tags"] for tag in filter_value)):
                    all_filters_match = False
                    break
            elif filter_key == "date_range":
                doc_date_str = doc_metadata.get("date")
                if doc_date_str and isinstance(filter_value, tuple) and len(filter_value) == 2:
                    try:
                        ...
                        all_filters_match = False
                        break
                elif doc_date_str != filter_value:  # For exact date match
                    all_filters_match = False
                    break
            else:
            ...
            if field:
                if field == "date" and order:
                    ...
                    if doc_date_str:
                        try:
                            doc_date = datetime.datetime.strptime(doc_date_str, "%Y-%m-%d").date()
                            # Convert date to a comparable number (e.g., days since epoch)
                            # This allows sorting by date numerically
                            doc_date_score = doc_date.toordinal()
                            doc.metadata["_date_score"] = doc_date_score
                        except ValueError:
...
if __name__ == "__main__":
    ...
    priorities3 = [{"field": "category_name", "value": "Open Government", "boost": 10}, {"field": "date", "order": "desc"}]
    prioritized_docs3 = prioritize_documents_by_metadata(documents, priorities3)
    for doc in prioritized_docs3:
        category_name = doc.metadata.get("categories")[0]["name"] if doc.metadata.get("categories") else "N/A"
