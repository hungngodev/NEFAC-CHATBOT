    ...
    if isinstance(results, list):
        for row in results:
            ...
            if isinstance(row, dict):
                meta = {k: v for k, v in row.items() if k not in ["text", "content"]}
...
# --- Interface for Ensemble Retriever ---


class GraphRetrieverInterface(BaseRetriever):
    """
    Interface wrapper for graph retrieval to work with ensemble retrievers.
    ...
    def _get_relevant_documents(self, query: str, **kwargs) -> List[Document]:
        ...
        class MinimalState:
            def __init__(self, query: str, state: Dict[str, Union[str, int, float, bool, List]]):
                ...
                self.statistical_query = state.get("statistical_query")

        minimal_state = MinimalState(query, self.state)

        # Call the main graph retrieval agent
        result = graph_retrieval_agent(minimal_state)
