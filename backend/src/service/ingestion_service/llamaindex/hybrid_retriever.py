...
class HybridRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        top_k: int = 10,
        rerank_top_n: int = 5,
        rerank_model: str = HYBRID_RERANK_MODEL,
        graph_query_fn: Optional[Callable[[QueryBundle], List[NodeWithScore]]] = None,
        fusion_strategy: str = HYBRID_FUSION_STRATEGY,
        ...
        self.enable_rerank = enable_rerank and bool(COHERE_API_KEY)
        self.top_k = top_k
        self.rerank_top_n = rerank_top_n
        self.rerank_model = rerank_model or HYBRID_RERANK_MODEL
        self.graph_query_fn = graph_query_fn

        base_weights = HYBRID_WEIGHTS.copy()
        if weights:
            base_weights.update(weights)
        self.weights = base_weights

        self.fusion_strategy = (fusion_strategy or HYBRID_FUSION_STRATEGY or "rrf").lower()
        self.rrf_k = rrf_k if rrf_k is not None else HYBRID_RRF_K
        self.min_score = min_score if min_score is not None else HYBRID_MIN_SCORE
    ...
    def _fuse_nodes(self, channel_results: Dict[str, List[NodeWithScore]]) -> List[NodeWithScore]:
        ...
        fused = [NodeWithScore(node=exemplars[key].node, score=score) for key, score in aggregate_scores.items() if score >= self.min_score]
...
class MultiDocHybridRetriever(HybridRetriever):
    ...
    def __init__(self, vector_index: VectorStoreIndex, doc_vector_index: Optional[VectorStoreIndex] = None, doc_weight: float = 0.3, chunk_weight: float = 0.7, **kwargs):
...
def create_hybrid_retriever(vector_index: VectorStoreIndex, mode: str = "hybrid", **kwargs) -> BaseRetriever:
