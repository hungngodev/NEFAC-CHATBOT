...
class LoaderService:
    ...
    def load(self, file_type, metadata_json_path):
        self.logger.info(
            f"Starting to load documents of type '{file_type}' from {metadata_json_path}"
        )
        docs = []
        if file_type == "pdf":
            docs = pdf_loader(
            ...
            return []
        self.logger.info(
            f"Loaded {len(docs)} documents of type '{file_type}' from {metadata_json_path}"
        )
        return docs
...
def main_pipeline(
    ...
    logger.info(
        f"Starting main pipeline for file_type='{file_type}', metadata_json_path='{metadata_json_path}'"
    )

    # 3. Graph RAG ingest step
    def graph_rag_runnable(docs: List[Document]) -> List[Document]:
        logger.info(
            f"[GraphRAG] Starting graph RAG ingestion for {len(docs)} documents..."
        )
        tqdm.write(
            f"[GraphRAG] Ingesting {len(docs)} documents into Neo4j knowledge graph..."
        )
    ...
    def index_runnable(docs: List[Document]) -> List[Document]:
        logger.info(
            f"[Indexing] Starting contextual retriever indexing for {len(docs)} documents..."
        )
        tqdm.write(
            f"[Indexing] Indexing {len(docs)} documents into Qdrant and Elasticsearch..."
        )
        contextualize_and_index_documents(
    ...
    if (
        ...
        tqdm.write(
            f"[Pipeline] ERROR: Pipeline did not return expected result dict: {results}"
        )
        return 0
    if not results["index"]:  # type: ignore
        logger.warning(f"No documents processed for {metadata_json_path}. Skipping.")
        tqdm.write(
            f"[Pipeline] WARNING: No documents processed for {metadata_json_path}. Skipping."
        )
        return 0
